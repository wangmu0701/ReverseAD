...
size_t kMIN_OP_PER_CP_FIXED = 1000000; // 1M
...
IterativeFuncFixed::IterativeFuncFixed(
  ...
  _num_iter = num_iter;
  // Use default value
  _min_op_per_cp = kMIN_OP_PER_CP_FIXED;
}

void IterativeFuncFixed::set_min_op_per_cp(size_t min_op_per_cp) {
  this->_min_op_per_cp = min_op_per_cp;
}

void IterativeFuncFixed::run(double* x_values, size_t x_num,
...
std::shared_ptr<DerivativeTensor<size_t, double>> IterativeFuncFixed::compute(
  ...
  if (t_order == 1) {
    ...
    std::cerr << "Only (1-3) orders for IterativeFuncFixed." << std::endl;
  }
