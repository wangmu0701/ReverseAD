...
CheckpointTrace::CheckpointTrace() {
  _checkpoint_num = 0;
  _tape_x_num = std::make_shared<TrivialTape<int>>();
  _tape_iter_num = std::make_shared<TrivialTape<int>>();
...
int CheckpointTrace::make_checkpoint(
    adouble* x, int x_num, std::shared_ptr<RuntimeEnv>& runtime_env) {
  _checkpoint_num++; 
...
void CheckpointTrace::set_iteration_num(int iter_num) {
  _tape_iter_num->put(iter_num);
}
int CheckpointTrace::get_iteration_num() {
  return _tape_iter_num->get_next_r();
}
int CheckpointTrace::get_checkpoint(
   adouble* x, int x_num, std::shared_ptr<RuntimeEnv>& runtime_env) {
