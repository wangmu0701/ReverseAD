  ...
  BaseActive<Base>& operator *= (const BaseActive<Base>& rhs) {
    ...
    trace_put_olllbb<Base>(mult_a_a, resloc, this->loc, rhs.loc, this->val, rhs.val);
    this->val *= rhs.val;
    this->loc = resloc;
    return *this;
  }
  ...
  BaseActive<Base>& operator *= (const double& rhs) {
    locint resloc = get_next_loc();
    trace_put_olld<Base>(mult_d_a, resloc, this->loc, rhs);
    this->val *= rhs;
    this->loc = resloc;
    return *this;
  }
  ...
  friend BaseActive<Base>&& operator*(BaseActive<Base>&& lhs, const BaseActive<Base>& rhs) {
    locint resloc = get_next_loc();
    trace_put_olllbb<Base>(mult_a_a, resloc, lhs.loc, rhs.loc, lhs.val, rhs.val);
    lhs.val *= rhs.val;
    lhs.loc = resloc;
#ifdef REVERSEAD_BASE_ACTIVE_DEBUG
  ...
  friend BaseActive<Base>&& operator*(BaseActive<Base>&& lhs, const double& val) {
    lhs.val *= val;
    locint resloc = get_next_loc();
    trace_put_olld<Base>(mult_d_a, resloc, lhs.loc, val);
    lhs.loc = resloc;
#ifdef REVERSEAD_BASE_ACTIVE_DEBUG
  ...
  friend BaseActive<Base>&& operator*(const double& val, BaseActive<Base>&& rhs) {
    rhs.val *= val;
    locint resloc = get_next_loc();
    trace_put_olld<Base>(mult_d_a, resloc, rhs.loc, val);
    rhs.loc = resloc;
#ifdef REVERSEAD_BASE_ACTIVE_DEBUG
    logger.info << "R(" << &rhs <<")[" << resloc << "] = " 
