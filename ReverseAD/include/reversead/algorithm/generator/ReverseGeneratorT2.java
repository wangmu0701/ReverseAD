...
public class ReverseGeneratorT2 {
  ...
  public boolean isGeneratingFuncBody; // true->body, false->case call
 
  public ReverseGeneratorT2() {
    indent = 0;
    fout = null;
  }
  ...
  public void generateUnarySingleFunc(int d, int r, int xC) {
    ...
    if (!isGeneratingFuncBody) {
      String caseLine = "case " + caseCode + " : ";
      dumpToFile(caseLine);
      dumpToFile("  {");
      dumpToFile("    unary_case_"+caseCode + 
          "(Z, sw, ginfo, global_deriv);");
      dumpToFile("  }");
      dumpToFile("  break;");
    } else {
      dumpToFile("template <typename Base>");
      String funcDecl = "void unary_case_"+caseCode + 
  ...
  public void generateUnary() {
   ...
   if (!isGeneratingFuncBody) {
    indent -= 2;
    dumpToFile("  default:");
    dumpToFile("    std::cout << \"unrecongized case code = \" << caseCode << std::endl;");
    dumpToFile("}");
    indent -= 2;
    dumpToFile("}");
    dumpToFile("");
  ...
  public void binaryRecursion(int curr_level, int max_level,
    ...
    for (int i = prev_index; i < BINARY_XY.length; i++) {
  ...
  public void generateBinarySingleFunc(int d, int r, int xC, int yC) {
    ...
    if (!isGeneratingFuncBody) {
      ...
      dumpToFile(caseLine);
      dumpToFile("  {");
      dumpToFile("    binary_case_" + caseCode +
                 "(Z, sw, ginfo, global_deriv);");
      dumpToFile("  }");
      dumpToFile("  break;");
      return;
      ...
      dumpToFile("template <typename Base>");
      String funcDecl = "void binary_case_" + caseCode + 
          "(const TensorIndex<locint>& Z, const Base& sw, " +
          "const TensorDerivativeInfo<locint, Base>& ginfo, " +
          "TensorDeriv<locint, Base>& global_deriv) {";
      dumpToFile(funcDecl);
  ...
  public void doWork() {
    try {
      fout = new PrintWriter(
          new BufferedWriter(new FileWriter("./../tensor2.ipp")));
      isGeneratingFuncBody = true;
      generateUnary();
      generateBinary();
      isGeneratingFuncBody = false;
      generateUnary();
      generateBinary();
      fout.close();
    } catch (Exception e) {
